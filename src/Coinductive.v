...
(* Fixpoint bad (u : unit) : Prop := bad u. *)
(* Inductive term : Set :=
| App : term -> term -> term
| Abs : (term -> term) -> term. *)
...
(*Fixpoint needs at least one parameter in the following example. And that's why we need CoInductive*)

(* Variable B : Type.
Inductive stream_ : Type :=
| Cons_ : B -> stream_ -> stream_.
Fixpoint zeros_ : stream nat := Cons_ 0 zeros_. *)
